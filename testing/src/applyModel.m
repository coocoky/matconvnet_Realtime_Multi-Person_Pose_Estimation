...
%     score{m} = applyDNN(imageToTest, net, twoLevel);
    ...
    net.eval({'image', gpuArray(single(imageToTest))});
    score{m} = gather(net.vars(net.getVarIndex('concat_stage7')).value);
    score{m} = permute(score{m}, [2 1 3]);
    score{m} = score{m}(:,:,[1:18,20:end]);
...
function img_out = preprocess(img, mean, param, click)
    ...
    img_out = img_out(:,:,[1 2 3]); % BGR for opencv training in caffe !!!!!
